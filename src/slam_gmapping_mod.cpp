...
laser_count_(0), transform_thread_(NULL)
{
  ...
  scan_filter_->registerCallback(boost::bind(&SlamGMapping::laserCallback, this, _1));*/
	...
	posestampedfile  = fopen("posestampedlog","rb");
	laserposefile = fopen("laserposelog","rb");
	minmaxfile = fopen("minmaxlog","rb");
...
SlamGMapping::initMapper(const sensor_msgs::LaserScan& scan)
{
	...
	parseLaserPoseStamped(laser_pose);
...
void
SlamGMapping::laserCallback(const sensor_msgs::LaserScan& scan)
{
...
void SlamGMapping::writeLaserMsgtoFile(const sensor_msgs::LaserScan & scan){
	...
	ROS_INFO_ONCE("size of *scan %lu\n",sizeof(scan));
	ROS_INFO_ONCE("size of sensor_msgs::LaserScan %lu\n",sizeof(sensor_msgs::LaserScan));
	ROS_INFO_ONCE ("header.seq: %u",(scan).header.seq);
	ROS_INFO_ONCE ("header.stamp: %u",(scan).header.stamp.nsec);
	//cout << "hea(.frame_id: "<<scan.header.frame_id << endl;
	ROS_INFO_ONCE ("angle_increment: %f ",(scan).angle_increment);
	ROS_INFO_ONCE ("angle_max: %f",(scan).angle_max);
	ROS_INFO_ONCE ("angle_min: %f",(scan).angle_min);
	ROS_INFO_ONCE ("range_max: %f",(scan).range_max);
	ROS_INFO_ONCE ("range_min: %f",(scan).range_min);
	ROS_INFO_ONCE ("ranges.size(): %lu",(scan).ranges.size());

	//Header

	fwrite(&scan.header.seq,sizeof(uint32_t),1,laserfile);
	fwrite(&scan.header.stamp.sec,sizeof(uint32_t),1,laserfile);
	fwrite(&scan.header.stamp.nsec,sizeof(uint32_t),1,laserfile);
...
tf::Stamped<tf::Transform> SlamGMapping::parseOdomMsgtoFile(){
	...
	fread(&quat,sizeof(tf::Quaternion),1,odomfile);
	fread(&vector,sizeof(tf::Vector3),1,odomfile);
...
sensor_msgs::LaserScan SlamGMapping::parseLaserMessage(){
...
void SlamGMapping::parsePoseStamped(tf::Stamped<tf::Pose> & pose){

	tf::Quaternion quat;
	tf::Vector3 origin;

	fread(&quat,sizeof(tf::Quaternion),1,posestampedfile);
	fread(&origin,sizeof(tf::Vector3),1,posestampedfile);
...
void SlamGMapping::parseMinMax(tf::Stamped<tf::Quaternion> & quat_stamped){
	tf::Quaternion quat;
	fread(&quat,sizeof(tf::Quaternion),1,minmaxfile);
	quat_stamped.setX(quat.getX());
	quat_stamped.setY(quat.getY());
	quat_stamped.setZ(quat.getZ());
	quat_stamped.setW(quat.getW());
}

void SlamGMapping::parseLaserPoseStamped(tf::Stamped<tf::Transform>& laserpose){
	tf::Quaternion quat;
	tf::Vector3 origin;
	fread(&quat,sizeof(tf::Quaternion),1,laserposefile);
	fread(&origin,sizeof(tf::Vector3),1,laserposefile);

	laserpose.setOrigin(origin);
	laserpose.setRotation(quat);

}

void SlamGMapping::testLoop(){

	while (!feof(laserfile)){
		const sensor_msgs::LaserScan scan=parseLaserMessage();
		laserCallback(scan);
	}
}
