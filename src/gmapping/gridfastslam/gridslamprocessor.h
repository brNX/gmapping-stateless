...
protected:
    ...
    int threadcounter;
    zmq::socket_t * workqueue;
    zmq::socket_t * syncqueue;
    void requester(const int id, const std::string & address);
    void localWorker(const int id);  
    /*******************NEW CODE *********************************/
...
#define isnan(x) (x==FP_NAN)
     ...
     void findUsedAreas(const int id, const int offset,std::pair<int,int>& minr ,std::pair<int,int>& minf,  std::pair<int,int>& maxr , std::pair<int,int>& maxf );
